		...
		switch (_run) do
		{
			...
			case 4:	{
						...
						if((DAC_Com_Values select 0) > 0) then
						{
							...
							{
								[format["%1 was deactivated",_group], {player groupchat _this}] remoteExec ["call", 0];
								//player groupchat format["%1 was deactivated",_group];
							}
							else
							{
								[format["%1 was deleted",_group], {player groupchat _this}] remoteExec ["call", 0];
			...
			case 8:	{
						if(_groupid < 2) then
						{
							...
							{
								DAC_InSpawn = false; publicVariable "DAC_InSpawn"; _run = 0;
							}
							else
							{
								_go = true;
								while {_go} do
								{
									...
									if(((getMarkerSize _markerVal) select 0) <= 4) then
									{
										DAC_InSpawn = false; publicVariable "DAC_InSpawn"; _run = 0;_go = false;
									}
									else
									{
										if(count _temparray1 == 0) then
			...
			case 10:{
						DAC_InSpawn = true; publicVariable "DAC_InSpawn"; _grouppara set[12,_group];
						if(!(_building in DAC_Camps_InUse)) then {DAC_Camps_InUse set [count DAC_Camps_InUse,_building]};
						if(((position _shet) select 0) == 0) then {_shet setpos (_building modelToWorld [0,15,0]);
							["Bad Spawn Position", {player groupchat _this}] remoteExec ["call", 0];
							//player groupchat "Bad Spawn Position"
						};
						...
						else
						{
							(((call compile _dc) select _rdmcamp) select 5) set [1,(((((call compile _dc) select _rdmcamp) select 5) select 1) - 1)];
							if((DAC_Com_Values select 0) > 0) then {
								[format["%1 was respawned",_group], {player groupchat _this}] remoteExec ["call", 0];
								//player groupchat format["%1 was respawned",_group]
							};
							sleep _rTime;
							if(_building in DAC_Camps_InUse) then {DAC_Camps_InUse = DAC_Camps_InUse - [_building]};
							_run = 0;
			...
			case 11:{
						[_group,_groupside,0] spawn DAC_fRemoveGroup;
						if((DAC_Com_Values select 0) > 0) then {
							[format["%1 was released from DAC",_group], {player groupchat _this}] remoteExec ["call", 0];
							//player groupchat format["%1 was released from DAC",_group]
						};
						_run = 0;
					};
			Default	{};
