...
else
{
	...
	if(_a == 3) then
	{
		if((({alive _x} count units _enemyGroup) > 0) || ((typeof _target) == "LOGIC")) then
		{
			...
			if((DAC_Com_Values select 0) > 0) then {if(format["%1",(side (leader _callGroup))] != "CIV") then {

				[format["Call arti support (%2x%4 %3) succesfully. Target to Player %1m",(_target distance player),count (_parameter select 1),side (leader _callGroup),_countbullets], {DAC_StartDummy globalchat _this}] remoteExec ["call", 0];

				//DAC_StartDummy globalchat format["Call arti support (%2x%4 %3) succesfully. Target to Player %1m",(_target distance player),count (_parameter select 1),side (leader _callGroup),_countbullets]
				}};
	...
	else
	{
		...
		if((DAC_Com_Values select 0) > 0) then {if(format["%1",(side (leader _callGroup))] != "CIV") then {
			[format["Call arti support [%2] aborted - reason %1",_h,side (leader _callGroup)], {DAC_StartDummy globalchat _this}] remoteExec ["call", 0];
			//DAC_StartDummy globalchat format["Call arti support [%2] aborted - reason %1",_h,side (leader _callGroup)]
			}};
