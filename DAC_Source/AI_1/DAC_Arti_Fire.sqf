			...
			else
			{
				if((alive _unit) && (!((group ((crew _unit) select 0)) in DAC_Fire_Groups))) then
				{
					...
					if(alive _unit) then
					{
						...
						{
							_run = false;
							[format["%1 > We have some problems > not ready to fire",_unit], {DAC_StartDummy globalchat _this}] remoteExec ["call", 0];
							//DAC_StartDummy globalchat format["%1 > We have some problems > not ready to fire",_unit];
						};
					}
					else
					{
