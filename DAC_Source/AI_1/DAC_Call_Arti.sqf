...
if(!(mission_AI_controller)) then
{
	...
	{
		["DAC_Support_Logic not found. Support for Players not available.", {player sideChat _this}] remoteExec ["call", 0];
	}
	else
	{
		if((DAC_Support_Logic getVariable "arti_count") >= (DAC_Player_Support select 0)) then
		{
			["Call arti support aborted - Max count arti support reached", {player sideChat _this}] remoteExec ["call", 0];
		}
		else
		{
...
else
{
	...
	if(count _artiArray > 0) then
	{
		...
		if(_c > 0) then
		{
			...
			if((typeName _target) == "ARRAY") then
			{
				if(format["%1",DAC_Support_Logic] == scalar) then
				{
					["DAC_Support_Logic not found. Support for Players not available.", {player sideChat _this}] remoteExec ["call", 0];_count = -1;_condi = [];
				}
				else
				{
			...
			if(count _condi > 0) then
			{
				if(call compile (_condi select 0)) then
				{
					if(count _condi > 1) then
					{
						if((DAC_Com_Values select 0) > 0) then {
							["Call arti support aborted - Max count arti support reached", {DAC_StartDummy globalchat _this}] remoteExec ["call", 0];
							//DAC_StartDummy globalchat (_condi select 1)
						};
				...
				else
				{
					...
					if((DAC_Com_Values select 0) > 0) then {if((side _unit) != "CIV") then {
						[format["Call arti support [%2] aborted - reason %1",11,side _unit], {DAC_StartDummy globalchat _this}] remoteExec ["call", 0];
						//DAC_StartDummy globalchat format["Call arti support [%2] aborted - reason %1",11,side _unit]
						}};_count = 0;
					sleep 1;
					if(!(isNil "DAC_Support_Logic")) then {DAC_Support_Logic setVariable ["arti_error", -1, true]};
				};
	...
	{
		["ERROR: arti config not found.", {DAC_StartDummy globalchat _this}] remoteExec ["call", 0];
