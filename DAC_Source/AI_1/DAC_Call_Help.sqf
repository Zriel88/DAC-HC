...
if(({alive _x} count units _grouponfire) > 0) then
{
	if((isPlayer _manonfire) && (count _this == 2)) then
	{
		if(!(isNil "DAC_Support_Logic")) then
		{
			...
			{
				["Call ground support aborted - No more ground support available", {player sideChat _this}] remoteExec ["call", 0];
			...
			else
			{
				...
				{
					["Call ground support aborted - Max count ground support reached", {player sideChat _this}] remoteExec ["call", 0];
				...
				else
				{
					if(!(mission_AI_controller)) then
					{
						...
						else
						{
							...
							[[_ma],8] spawn DAC_fFlashMarker;
							_action = false;
							["Position is outside the valid range", {player sideChat _this}] remoteExec ["call", 0];
									...
									if(!(isNil "DAC_Support_Logic")) then
									{
										if((count(DAC_Support_Logic getVariable "support_caller")) > 0) then
										{
											...
											if((DAC_Com_Values select 0) > 0) then {
												["Call ground support succesfully.", {player sideChat _this}] remoteExec ["call", 0];
