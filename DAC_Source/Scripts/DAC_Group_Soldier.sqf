...
else
{
	...
	while {_go > 0} do
	{
		...
		{
			_go = 0;DAC_Master_Init = DAC_Master_Init - 1;_run = 0;DAC_InSpawn = false; publicVariable "DAC_InSpawn"; 
		}
		else
		{
		...
		switch (_run) do
		{
			case 1:	{
						...
						else
						{
							if(((getMarkerSize _ma) select 0) <= 4) then
							{
								_run = 0;DAC_InCreate = false;DAC_InSpawn = false; publicVariable "DAC_InSpawn"; DAC_Master_Init = DAC_Master_Init - 1;
							}
							else
							{
			...
			case 10:{
						...
						DAC_InCreate = false;DAC_InSpawn = false; publicVariable "DAC_InSpawn"; DAC_Master_Init = DAC_Master_Init - 1;_run = 0;
					};
			Default{};
		};
	};
