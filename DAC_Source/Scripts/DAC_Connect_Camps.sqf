...
while {_find1 == 0} do
{
	...
	{
		case 3:	{
					[format["Error connect Camps: %1 is not a valid side value",_ZoneSide], {player groupchat _this}] remoteExec ["call", 0];
		...
		case 2:	{
					if(_countF == 0) then
					{
						[format["No Camp on side %1 are connected.",_SideTX select _SideID], {player groupchat _this}] remoteExec ["call", 0];
																					...
																					else
																					{
																						...
																						else
																						{
																							...
																							if(!((getPosATL _CheckZone select 0) in (((call compile _CampList) select _mn) select 6))) then
																							{
																								...
																								{
																									[format["(%4) DAC connects [%1_Camp_%2] with [Zone_%3]",_SideTX select _SideID,round (getPosATL (((call compile _CampList) select _mn) select 1) select 0),round (getPosATL _CheckZone select 0),_countF], {player groupchat _this}] remoteExec ["call", 0];
