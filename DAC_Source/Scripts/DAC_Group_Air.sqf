	...
	while {_run > 0} do
	{
		switch (_run) do
		{
			...
			case 5:	{
						...
						if((((getMarkerSize _ma) select 0) == 6) && (!(DAC_InSpawn)) && ((DAC_Com_Values select 0) > 0) && ((getdir DAC_StartDummy) > 170)) then {
							[format["%1 was activated",_group], {player groupchat _this}] remoteExec ["call", 0];
							//player groupchat format["%1 was activated",_group];
						};
