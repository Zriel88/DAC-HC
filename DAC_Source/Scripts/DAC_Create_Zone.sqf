...
else
{
	...
	else
	{
		...
		sleep (5 + (random 1));
		if((DAC_Com_Values select 0) > 0) then {if((_ZoneMode select 1) == 1) then {DAC_Inactive_Zones set [count DAC_Inactive_Zones,_KiZone];
			[format["Zone %1 is deactivated",_thisZone], {player groupchat _this}] remoteExec ["call", 0];
			//player groupchat format["Zone %1 is deactivated",_thisZone];
			}};
	};
};
