...
if(mission_AI_controller) then
{
	...
	{
		DAC_NewZone = 1; publicVariable "Dac_NewZone";
		...
		else
		{
			...
			if(count _mP > 0) then
			{
				if(_uc > 0) then
				{
					...
					{
						[format["Set new UnitConfig to zone [%1] = %2",round (getPosATL _zone select 0),_uc], {player groupchat _this}] remoteExec ["call", 0];
				...
				if(_bc > 0) then
				{
					...
					if((DAC_Com_Values select 0) > 0) then
					{
						[format["Set new BehaviourConfig to zone [%1] = %2",round (getPosATL _zone select 0),_bc], {player groupchat _this}] remoteExec ["call", 0];
				...
				DAC_NewZone = 0; publicVariable "Dac_NewZone";
			}
			else
			{
	...
	{
		if((DAC_Com_Values select 0) > 0) then {
			["Any Zone action in procress. Please try again later.", {player groupchat _this}] remoteExec ["call", 0];
