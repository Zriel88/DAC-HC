			...
			if((DAC_Com_Values select 0) > 0) then
			{
				...
				while{true} do
				{
					...
					if(format["%1",(DAC_Support_Logic getVariable "arti_error")] != "-1") then
					{
						...
						{
							["Call arti support succesfully.", {player sidechat _this}] remoteExec ["call", 0];
							//player sidechat "Call arti support succesfully.";
						}
						else
						{
							[format["Call arti support aborted - reason %1",(DAC_Support_Logic getVariable "arti_error")], {player sidechat _this}] remoteExec ["call", 0];
