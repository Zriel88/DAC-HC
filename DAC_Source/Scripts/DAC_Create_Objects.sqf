	...
	if(_DAC_Count_WP > 0) then
	{
		...
		if(DAC_Basic_Value != 10) then
		{
			...
			else
			{
				...
				while {_run > 0} do
				{
					switch (_run) do
					{
						case 3:	{
									...
									{
										if(count _wpConfig == 0) then {
											["DAC_Create_Object: bad wp_config", {player sidechat _this}] remoteExec ["call", 0];_run = 0;
											//player sidechat "DAC_Create_Object: bad wp_config";_run = 0;
										};
										if(count _objConfig == 0) then {
											["DAC_Create_Object: bad obj_config", {player sidechat _this}] remoteExec ["call", 0];_run = 0;
											//player sidechat "DAC_Create_Object: bad obj_config";_run = 0;
										};
									}
									else
									{
						...
						case 11:{
									...
									if(_run != 7) then
									{
										...
										else
										{
											...
											{
												_wpnr = _DAC_Count_WP;
												[format["Zone '%1' : To create positions only, you have to define an array",(_this select 0)], {player sidechat _this}] remoteExec ["call", 0];
						...
						case 12:{
									if((DAC_Com_Values select 0) > 0) then {
										[format["Zone '%1' : Only %2 of %3 objects could be generated.",(_this select 0),_wpnr,_DAC_Count_WP], {player sidechat _this}] remoteExec ["call", 0];
										//player sidechat format["Zone '%1' : Only %2 of %3 objects could be generated.",(_this select 0),_wpnr,_DAC_Count_WP];
									};
						...
						case 13:{
									if((DAC_Com_Values select 0) > 0) then {
										[format["Zone '%1' : All %2 objects were generated.",(_this select 0),_wpnr,_DAC_Count_WP], {player sidechat _this}] remoteExec ["call", 0];
										//player sidechat format["Zone '%1' : All %2 objects were generated.",(_this select 0),_wpnr,_DAC_Count_WP];
									};
									if(!(format["%1",_arrayName] == "")) then {_KiZone setVariable ["objCreated", call compile _arrayName]};
									sleep 1;
									{deletevehicle _x} foreach [_DumyA,_DumyB,_Null];
				...
				if(count _debA > 0) then {
					hintc format["Important Hint:\n\nBad object(s) in DAC_Config_Objects No. %1:\n\n%2\n\n\nIt's not possible to 'create' these objects.",_obj_Nr,_debA];
					["bad objects", {player sidechat _this}] remoteExec ["call", 0];
					//player sidechat "bad objects";
				};
			};
		};
	};
